...
product ::
  List Int
  -> Int
product list = foldLeft (\a b -> a * b) 1 list
...
filter ::
  ...
  -> List a
  -> List a
filter predi list = foldRight 
  (\a b -> if predi a then a :. b else b)
  Nil
  list
...
flatten ::
  List (List a)
  -> List a
flatten = foldRight (\val acc -> val ++ acc) Nil
...
find predi = 
  foldRight 
  (\val acc -> if (predi val) then Full val else acc)
  Empty
